...
"""
    check_if_ultroid_official = await is_user_on_chat(bot, "@ary_botz", m.peer_id)
    if not check_if_ultroid_official:
        await m.reply("Please join @ary_botz then send me the link again.")
        return

    check_if_ultroid_official_chat = await is_user_on_chat(bot, "@ary_botz", m.peer_id)
...
async def start(m: UpdateNewMessage):
    ...
    check_if = await is_user_on_chat(bot, "@ary_botz", m.peer_id)
    if not check_if:
        return await m.reply("Please join @ary_botz then send me the link again.")
    check_if = await is_user_on_chat(bot, "@ary_botz", m.peer_id)
    if not check_if:
        return await m.reply(
            "Please join @ary_botz then send me the link again."
        )
    await bot(
...
async def handle_message(m: Message):
    ...
    if is_spam and m.sender_id not in [5079629749]:
